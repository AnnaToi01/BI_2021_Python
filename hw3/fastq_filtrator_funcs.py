...
def qs_threshold(reads, dic_ind, quality_threshold=0):
    """
    Filters reads according to their quality score (phred33)
    quality_threshold - lowest mean quality score for the read
    """
    for i, read in enumerate(reads):
        quality_sum = 0
        for character in read[3]:
            quality_sum += ord(character) - 33
        quality_mean = quality_sum / len(read[3])
...
def main(input_fastq, output_file_prefix, gc_bounds=(0, 100), length_bounds=(0, 2 ** 32), quality_threshold=0,
    ...
    with open(input_fastq, 'r') as file, open(passed_reads, "w") as passed, open(failed_reads, "w") as failed:
        ...
        if quality_threshold != 0:
            qs_threshold(reads, dic_ind, quality_threshold)
...
if __name__ == "__main__":
    ...
    quality_threshold_in = input("Enter the minimal mean quality of the read: ")
    try:
    ...
    # quality_threshold_in = float("30")
    # save_filtered_in = bool(int("0"))
    kwargs = dict(gc_bounds=gc_bounds_in, length_bounds=length_bounds_in,
                  quality_threshold=quality_threshold_in, save_filtered=save_filtered_in)
